...
window.switchTab = function(mode) {
    ...
    document.getElementById('panelSolar').style.display = mode === 'solar' ? 'flex' : 'none';
    document.getElementById('panelLunar').style.display = mode === 'lunar' ? 'flex' : 'none';
    document.getElementById('panelGanZhi').style.display = mode === 'ganzhi' ? 'flex' : 'none';
}
...
window.initChart = function() {
    ...
    try {
        ...
        // 3. 計算真太陽時 (calculatingSolar) - 這是用來排八字的
        //    但我們保留 originSolar 用來顯示和儲存
        ...
        // 4. 填充儀表板 - 使用【原始時間 originSolar】來顯示
        ...
        // 5. 執行排盤 - 使用【計算後的 calculatingSolar】
        state.birthSolar = calculatingSolar; // 讓後續的流年運算基於真太陽時
