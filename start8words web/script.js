    ...
    try {
        ...
        if (calculatingSolar.getHour() === 23) {
            if (zishiMode === '23') {
                ...
                // 00:00 換日：確保不換日
                // 注意：部分 library 版本對於 23:00 會自動換日，這裡我們手動設定回當日
                // 但為了保險，我們先信任 library 的輸出，如果發現錯了再手動 new Date 調整
                // 此處保持原邏輯，視為夜子
                isNightRat = true;
            }
        }
        ...
        window.currentBaziData = {
            ...
            // 【修正】統一使用 "客戶" (標準繁體)
            tags: document.getElementById('tagsInput') ? document.getElementById('tagsInput').value : '客戶', 
            zishiMode: zishiMode,
            bazi: {
